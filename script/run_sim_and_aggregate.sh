...
# ★ここを追加：スクリプト実行時に古いCSVが残っていれば削除（リセット）する
...
# 3. バッチ処理ループ
# ==========================================
# 0からTOTAL_FILESまで、BATCH_SIZEずつ増やしながらループ
for (( i=0; i<$TOTAL_FILES; i+=$BATCH_SIZE )); do
    
    # 今回のバッチで処理するファイルを配列として切り出す
    ...
    # 切り出したファイル群を xargs で並列実行
    printf "%s\n" "${BATCH_FILES[@]}" | xargs -P 7 -n 1 /opt/scenargie_simulator/source/driot/sim
